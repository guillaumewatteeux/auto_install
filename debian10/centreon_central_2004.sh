...
" | tee -a ${INSTALL_LOG}
  ...
  if [ ! -f /etc/apt/sources.list.d/buster-nonfree.list ]; then
...
" | tee -a ${INSTALL_LOG}
  ...
  dpkg -l | grep "mariadb-server-10.3" >> ${INSTALL_LOG}
  if [ $? -ne 0 ]; then
    apt install -y mariadb-server >> ${INSTALL_LOG}

    [ "$SCRIPT_VERBOSE" = true ] && echo "====> Configure mariadb" | tee -a ${INSTALL_LOG}
    # Add mysql config for Centreon
	cat << __EOT__ > /etc/mysql/conf.d/centreon.cnf
#
...
" | tee -a ${INSTALL_LOG}
  ...
  apt install -y wget cmake python3-pip >> ${INSTALL_LOG}

  cd ${DL_DIR}
  if [[ -e centreon-clib-${CLIB_VER}.tar.gz ]]; then
...
" | tee -a ${INSTALL_LOG}
  ...
  apt install -y libperl-dev libssh2-1-dev libgcrypt-dev >> ${INSTALL_LOG}
  pip3 install conan >> ${INSTALL_LOG}
  /usr/local/bin/conan remote add centreon https://api.bintray.com/conan/centreon/centreon >> ${INSTALL_LOG}

  cd ${DL_DIR}
  if [[ -e centreon-connectors-${CONNECTOR_VER[0]}.tar.gz ]]; then
    echo 'File already exist !' | tee -a ${INSTALL_LOG}
  else
...
" | tee -a ${INSTALL_LOG}
  ...
  if [[ $MAJOUR == 2 ]]; then
    ...
    #verify permission
    if [ $(ls -l "/var/lib/centreon-engine" | awk '{print $3}') == "root" ]; then
...
a pt install -y libcgsi-gsoap-dev zlib1g-dev libssl-dev libxerces-c-dev >> ${INSTALL_LOG}

# Cleanup to prevent space full on /var
apt-get clean >> ${INSTALL_LOG}

  cd ${DL_DIR}
  if [[ -e centreon-engine-${ENGINE_VER[0]}.tar.gz ]]; then
    echo 'File already exist !' | tee -a ${INSTALL_LOG}
  else
...
" | tee -a ${INSTALL_LOG}
  ...
  apt install --force-yes -y libgnutls28-dev libssl-dev libkrb5-dev libldap2-dev libsnmp-dev gawk \
  ...
  cd ${DL_DIR}
  if [[ -e monitoring-plugins-${PLUGIN_VER}.tar.gz ]]; then
    echo 'File already exist !' | tee -a ${INSTALL_LOG}
  else
  ...
  ./configure --with-nagios-user=${ENGINE_USER} \
    --with-nagios-group=${ENGINE_GROUP} \
...
" | tee -a ${INSTALL_LOG}
  ...
  cd ${DL_DIR}
  DEBIAN_FRONTEND=noninteractive apt install -y --force-yes libxml-libxml-perl libjson-perl libwww-perl libxml-xpath-perl \
  ...
  cd ${DL_DIR}
  if [[ -e centreon-plugins-${PLUGIN_CENTREON_VER[0]}.tar.gz ]]; then
    echo 'File already exist !' | tee -a ${INSTALL_LOG}
  else
...
" | tee -a ${INSTALL_LOG}
  ...
  if [[ $MAJOUR == 2 ]]; then
    ...
    #verify permission
    if [ $(ls -l "/var/lib/centreon-broker" | awk '{print $3}') == "root" ]; then
  ...
  apt install -y git librrd-dev libmariadb-dev libgnutls28-dev lsb-release liblua5.2-dev >> ${INSTALL_LOG}

  # Cleanup to prevent space full on /var
  apt-get clean >> ${INSTALL_LOG}

  cd ${DL_DIR}
  if [[ -e centreon-broker-${BROKER_VER[0]}.tar.gz ]]; then
    echo 'File already exist !' | tee -a ${INSTALL_LOG}
  else
...
" | tee -a ${INSTALL_LOG}
  ...
  if [[ $MAJOUR == 2 ]]; then
    apt install -y php php7.3-opcache libapache2-mod-php php-mysql \
            ...
            libsnmp-perl snmpd snmptrapd snmp-mibs-downloader >> ${INSTALL_LOG}
...
" | tee -a ${INSTALL_LOG}
  ...
  if [[ $MAJOUR == 2 ]]; then
    apt install -y libzmq3-dev libssh-dev libextutils-makemaker-cpanfile-perl \
              libmodule-build-perl libmodule-install-perl >> ${INSTALL_LOG}
  ...
  cd ${DL_DIR}
  if [[ -e centreon-gorgone-${GORGONE_VER[0]}.tar.gz ]]; then
    echo 'File already exist !' | tee -a ${INSTALL_LOG}
  else
  ...
  cat << __EOT__ > ${DL_DIR}/${GORGONE_TMPL}
# gorgone template
# deployment paths
...
" | tee -a ${INSTALL_LOG}
  ...
  if [[ -e ${PREFIX}${CENTREON_VER[0]}.tar.gz ]]; then
    echo 'File already exist!' | tee -a ${INSTALL_LOG}
  else
...
" | tee -a ${INSTALL_LOG}
  ...
  # add node-js
  apt install -y curl >> ${INSTALL_LOG}
  curl -sL https://deb.nodesource.com/setup_12.x | bash - >> ${INSTALL_LOG}
  apt install -y nodejs >> ${INSTALL_LOG}
...
EOF
  ...
  c at > /etc/centreon-gorgone/config.d/30-centreon.yaml <<EOF
name: centreon.yaml
...
EOF
    ...
    sed -i -e "s/unshift \$register_subnodes->{\$_->\[0\]}/unshift \@{\$register_subnodes->{\$_->\[0\]}}/g" \
      /usr/share/perl5/gorgone/modules/centreon/nodes/class.pm

    systemctl restart gorgoned

    #add cgroup centreon
    cat << __EOT__ > /lib/systemd/system/centreon.service
[Unit]
...
__EOT__
    ...
    find /var/lib/centreon/sessions/* | /usr/bin/xargs rm -f
    #add conf
    cat > /etc/centreon/config.d/10-database.yaml <<EOF
database:
  db_configuration:
...
" | tee -a ${INSTALL_LOG}
  ...
  sed -i -e "s/\$res = \$db->query(\$queryPEND);/#\$res = \$db->query(\$queryPEND);/g" \
    ${INSTALL_DIR}/centreon/www/widgets/tactical-overview/src/hosts_status.php
}
...
" | tee -a ${INSTALL_LOG}
  ...
  apt install -y libssl-dev >> ${INSTALL_LOG}	
  cd ${DL_DIR}
  ...
  ./configure --with-nagios-user=${ENGINE_USER} \
    --with-nrpe-user=${ENGINE_USER} \
    --with-nagios-group=${ENGINE_USER} \
    --with-nrpe-group=${ENGINE_USER} \
    --libexecdir=/usr/lib/nagios/plugins  >> ${INSTALL_LOG}
...
maj_conf () {
	cat /etc/centreon/install_auto.conf | grep "^$1="  >> ${INSTALL_LOG}
	if [[ $? -ne 0 ]];
	then
...
exist_conf () {
	if [ ! -f /etc/centreon/install_auto.conf ]; then
